...
extern bool __ob_util_openfile(FILE **fp, const char *const path,
...
bool __ob_log_initmodule(void) {
  ...
  if (__ob_log_getlogfilename(&logfile_name, &logfile_namelen))
    if (!__ob_log_openlogfile(logfile_name)) {
      ...
      printf("[OBSIDIAN:FATAL] Failed to open a log file! Required by Obsidian "
             "for proper functionality and debugging!\n");
      free(logfile_name);
      logfile_name = NULL;
      return false;
    }
...
bool __ob_log_getlogfilename(char **buffer, size_t *bufferSize) {
  ...
  (*bufferSize) = ydigits + mdigits + ddigits + strlen(".log") +
                  3; // +2 for '.' and + 1 for \0
...
bool OBLOGcloseLogFile(void) { return false; }

bool OBLOGwriteHeader(void) { return false; }
